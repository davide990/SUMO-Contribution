...
# Modified by D. Guastella, 15-4-2026
...
def findNotifcationEdges(options, net, closedEdges):
    ...
    for e in closedEdges:
        ...
        # queue holds (edge, depth)
        upstream = deque()
        for pred in e.getIncoming().keys():
            if pred.allows(options.vclass):
                upstream.append((pred, 1))

        seen = set()
        while upstream:
            cand, depth = upstream.popleft()
            if cand in seen:
                continue
            seen.add(cand)

            # Check if this upstream edge can reach the downstream region
            reachable2 = net.getReachable(cand, options.vclass)
            if reachable2.intersection(reachable):
                result.add(cand)

            # Stop at depth 1 (this can be varied to explore further levels)
            if depth >= 1:
                continue

            # Add next-level upstream edges
            for pred in cand.getIncoming().keys():
                if pred.allows(options.vclass) and pred not in seen:
                    upstream.append((pred, depth + 1))
